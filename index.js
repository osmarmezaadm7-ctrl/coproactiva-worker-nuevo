...
// ── Usuarios bootstrap locales ───────────────────────────────────────────────
// Se usan como respaldo del login de Apps Script para permitir accesos iniciales.
// Las claves NO se guardan en texto plano: se compara SHA-256.
const BOOTSTRAP_USERS = [
  {
    id: 'usr_contacto_coproactiva',
    nombre: 'CoproActiva',
    email: 'contacto@coproactiva.cl',
    rol: 'superadmin',
    modulos: ['*'],
    comunidades: ['*'],
    passwordHash: 'dd89d967141a55cf0ff047cde7fa73658e05ee90182e32e5b0d8e4a812b6ea11',
  },
  {
    id: 'usr_osmar_meza',
    nombre: 'Osmar Meza',
    email: 'osmarmeza.adm7@gmail.com',
    rol: 'superadmin',
    modulos: ['*'],
    comunidades: ['*'],
...
async function validarUsuarioBootstrap(email, clave) {
  const emailNorm = String(email || '').trim().toLowerCase();
  const usuario = BOOTSTRAP_USERS.find(u => u.email === emailNorm);
  if (!usuario) return null;
  const claveHash = await sha256Hex(clave);
  if (claveHash !== usuario.passwordHash) return false;
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/auth' && method === 'POST') {
      try {
        ...
        const usuarioBootstrap = await validarUsuarioBootstrap(body.email, body.clave);
        if (usuarioBootstrap === false) {
          return jsonResponse({ ok: false, error: 'Credenciales incorrectas' }, 401, origin);
        }

        let usuario = usuarioBootstrap;
        if (!usuario) {
          const resp = await postAppsScript(APPS_SCRIPT_URL, {
            action: 'loginUsuario',
            email:  body.email,
    ...
    const userId = await validarToken(token, SECRET_KEY);
    ...
    try {
      ...
      // ── v3.7 — Scraping indicadores Previred (via GAS) ──────────────────
      if (url.pathname === '/remuneraciones/parametros/scraping-previred' && method === 'GET') {
        const data = await callAppsScript(APPS_SCRIPT_URL, 'scrapingIndicadoresPrevired');
        return jsonResponse({ ok: true, data }, 200, origin);
      }
